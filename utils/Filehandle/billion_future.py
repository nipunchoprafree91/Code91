...
class FilesystemLoader:
    ...
    def create_directory_structure(self, path, level):
        ...
        with ThreadPoolExecutor(max_workers=self.width) as executor:
            futures = []
            for i in range(self.width):
                subdir = os.path.join(path, f"dir{i}_{self.depth+1}")
                if not os.path.exists(subdir):
                    os.makedirs(subdir)
                futures.append(executor.submit(self.create_directory_structure, subdir, level-1))
                futures.append(executor.submit(self.create_files, subdir))
    ...
    def load_filesystem(self):
        ...
        print("Filesystem loaded successfully in {:.2f} seconds.".format(total_time))


if __name__ == "__main__":
    filesystem_loader = FilesystemLoader(ROOT_DIR, DEPTH, WIDTH, NUM_FILES_PER_DIRECTORY)
